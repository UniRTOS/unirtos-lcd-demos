...
This demo demonstrates the basic development workflow for LCD display control on UniRTOS, and is suitable as a beginner example for LCD peripheral debugging and GUI application development.

- Demonstrates LCD initialization and screen clearing with ST7567 and ST7789V driver support
- Demonstrates image rendering by writing pixel data (e.g. UniRTOS logo) to the display
- Includes a refresh rate benchmark test that measures average FPS over a 5-second animation run
- Supports multiple test cases switchable via the `g_lcd_test_case` configuration variable
- Easily extensible to text rendering, UI widgets, real-time data visualization, and multi-screen management
